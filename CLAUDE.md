...
- The live homepage intro is `SupercutIntro` (supercut-intro.tsx): a rectangle eases in at 80% of the viewport and supercuts IN PLACE (speed-ramped, halftone overlay) for ~1s, then splits right there, seamlessly (quadrant shards of the current frame), into four quarter pieces that each CONTINUE supercutting their own dealt frame stream while carrying the whole journey down onto the grid's first four cards on a gentle arc, resolving to live previews as they seat (`[data-supercut-target]` index-stamped in video-grid.tsx; pieces land on the `data-supercut-media` 16:9 box inside VideoCard, not the whole cell). Intro-context signals: `introTargetCount` = how many cards the grid holds back; `introLandedCount` = the ENTIRE grid reveals once the last piece seats (targets as one simultaneous fade, lower rows with their stagger); `introHeroReveal` = the hero fades 50%→100% once all pieces have landed, same beat. Scrolling is locked (html overflow) until the intro's 'done' phase. No audio (autoplay policy makes it impossible pre-gesture). When the four cards aren't all on screen (single-column mobile) it lands on card one alone. Design iterations live on the `/supercut` sandbox route (with an effects panel, incl. the dropped CRT power-on).
